...
local GetNumFactions, GetFactionInfo, ExpandFactionHeader, CollapseFactionHeader = C_Reputation.GetNumFactions, C_Reputation.GetFactionDataByIndex, ExpandFactionHeader, CollapseFactionHeader
local GetFactionInfoByID, IsInGuild, GetGuildInfo = C_Reputation.GetFactionDataByID, IsInGuild, GetGuildInfo
...
do 
	...
	local function AddFaction(id, text)
		text = text or C_Reputation.GetFactionDataByID(id)
		factions[id] = text
...
local function SaveHeaders()
	...
	for i = C_Reputation.GetNumFactions(), 1, -1 do		-- 1st pass, expand all categories
		local name, _, _, _, _, _, _,	_, isHeader, isCollapsed = C_Reputation.GetFactionDataByIndex(i)
		if isHeader then
	...
	-- for i = C_Reputation.GetNumFactions(), 1, -1 do
		-- if IsFactionInactive(i) then
			-- local name = C_Reputation.GetFactionDataByIndex(i)
			-- inactive[name] = true
			-- SetFactionActive(i)
		-- end
	-- end
...
local function RestoreHeaders()
	local headerCount = 0
	for i = C_Reputation.GetNumFactions(), 1, -1 do
		local name, _, _, _, _, _, _,	_, isHeader = C_Reputation.GetFactionDataByIndex(i)
...
local function ScanSingleFaction(factionID)
	...
	local value
	local _, _, standing, _, _, earned = C_Reputation.GetFactionDataByID(factionID)
...
local function ScanAllFactions()
	...
	for i = 1, C_Reputation.GetNumFactions() do
		local factionID = select(14, C_Reputation.GetFactionDataByIndex(i))
...
local function ScanGuildReputation()
	...
	SaveHeaders()
	for i = 1, C_Reputation.GetNumFactions() do		-- 2nd pass, data collection
		local name, _, standing, _, _, earned = C_Reputation.GetFactionDataByIndex(i)
